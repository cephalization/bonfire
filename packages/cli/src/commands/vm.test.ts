...
describe("parseVMCreateArgs", () => {
  ...
  it("parses all flags together", () => {
    const result = parseVMCreateArgs([
      "my-vm",
      "--vcpus=2",
      "--memory=1024",
      "--image=debian:12",
  ...
  it("throws on invalid --vcpus value", () => {
    expect(() => parseVMCreateArgs(["my-vm", "--vcpus=0"])).toThrow(
      "Invalid --vcpus value"
    );
    expect(() => parseVMCreateArgs(["my-vm", "--vcpus=abc"])).toThrow(
      "Invalid --vcpus value"
    );
    expect(() => parseVMCreateArgs(["my-vm", "--vcpus=-1"])).toThrow(
      "Invalid --vcpus value"
    );
  });

  it("throws on invalid --memory value", () => {
    expect(() => parseVMCreateArgs(["my-vm", "--memory=0"])).toThrow(
      "Invalid --memory value"
    );
    expect(() => parseVMCreateArgs(["my-vm", "--memory=abc"])).toThrow(
      "Invalid --memory value"
    );
    expect(() => parseVMCreateArgs(["my-vm", "--memory=-1"])).toThrow(
      "Invalid --memory value"
    );
  });
